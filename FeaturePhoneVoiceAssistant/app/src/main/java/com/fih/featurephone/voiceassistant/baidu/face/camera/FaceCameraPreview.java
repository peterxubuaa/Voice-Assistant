...
import com.fih.featurephone.voiceassistant.baidu.face.callback.CameraDataCallback;
...
public class FaceCameraPreview extends FrameLayout {
    ...
    private volatile boolean mEnableFaceDetect = false;
    ...
    private void init() {
        ...
        if (CommonUtil.isSupportMultiTouch(getContext())) {
            SingleBaseConfig.getBaseConfig().setDetectDirection(90);
        } else {
            SingleBaseConfig.getBaseConfig().setDetectDirection(270);
        }
        SingleBaseConfig.getBaseConfig().setMirrorRGB(0);
...
//        super.onLayout(changed, left, top, right, bottom); must remove
    ...
    private void faceDetect(byte[] data, final int width, final int height) {
        // 摄像头预览数据进行人脸检测
        FaceSDKManager.getInstance().onDetectCheck(data, width, height, new FaceDetectCallBack() {
            @Override
