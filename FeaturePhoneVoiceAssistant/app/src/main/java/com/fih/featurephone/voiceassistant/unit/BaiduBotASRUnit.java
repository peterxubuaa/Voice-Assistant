...
import com.fih.featurephone.voiceassistant.unit.parsejson.ParseASRJson;
import com.fih.featurephone.voiceassistant.unit.parsejson.ParseJson;
...
//        } else if(name.equals("asr.partial")) {
    ...
    private void getBestResponseFromBotUnit(ParseASRJson.BotSimpleResponse botSimpleResponse, BaiduUnitAI.BestResponse bestResponse) {
        ...
        if (TextUtils.isEmpty(bestResponse.mBotID)) {
            //第一次对话，从中找关键字，区分对话类别
            String[] result = mBaiduUnitAI.getBestBotID(bestResponse.mRawQuery);
            bestResponse.mBotID = result[0];
            if (!TextUtils.isEmpty(bestResponse.mBotID)) {
