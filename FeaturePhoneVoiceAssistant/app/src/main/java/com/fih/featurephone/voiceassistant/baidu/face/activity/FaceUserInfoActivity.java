...
public class FaceUserInfoActivity extends Activity {
    ...
    private final int REQUEST_LOAD_IMAGE = 100;
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (Activity.RESULT_OK == resultCode && REQUEST_LOAD_IMAGE == requestCode) {
            String faceImageFilePath = SystemUtil.getAlbumImagePath(this, data.getData());
            if (FileUtils.isFileExist(faceImageFilePath)) {
                mUpdateUerImage = true;
                int rotateDegree = BitmapUtils.decodeImageDegree(faceImageFilePath);
                ((ImageView) findViewById(R.id.user_image_image_view))
                        .setImageBitmap(BitmapUtils.rotateBitmap(rotateDegree, BitmapFactory.decodeFile(faceImageFilePath)));
            }
        }
    }
            ...
            public void onClick(DialogInterface dialog, int which) {
                switch (which) {
                    case 0://camera
                        ...
                        break;
                    case 1://image
                        SystemUtil.startSysAlbumActivity(FaceUserInfoActivity.this, REQUEST_LOAD_IMAGE);
                        break;
                    default:
                        break;
                }
