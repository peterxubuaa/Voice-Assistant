package com.fih.featurephone.voiceassistant.baidu.speech.listener;

import android.util.Log;

import com.baidu.speech.EventListener;
import com.baidu.speech.asr.SpeechConstant;
import com.fih.featurephone.voiceassistant.baidu.speech.RecogResult;
...
public class RecogEventAdapter implements EventListener {
    ...
    private IRecogListener mListener;
...
//        }
        if (name.equals(SpeechConstant.CALLBACK_EVENT_ASR_LOADED)) {
            ...
            if (recogResult.isFinalResult()) {
                // 最终识别结果，长语音每一句话会回调一次
                mListener.onAsrFinalResult(results, recogResult);
            } else if (recogResult.isPartialResult()) {
                // 临时识别结果
                mListener.onAsrPartialResult(results, recogResult);
            } else if (recogResult.isNluResult()) {
                // 语义理解结果
                mListener.onAsrOnlineNluResult(new String(data, offset, length));
            }
...
//                MyLogger.error(TAG, "asr error:" + params);
                mListener.onAsrFinishError(errorCode, subErrorCode, recogResult.getDesc(), recogResult);
            } else {
                mListener.onAsrFinish(recogResult);
            }
        } else if (name.equals(SpeechConstant.CALLBACK_EVENT_ASR_LONG_SPEECH)) { // 长语音
            mListener.onAsrLongFinish(); // 长语音
            ...
            mListener.onAsrVolume(vol.mVolumePercent, vol.mVolume);
            ...
            mListener.onAsrAudio(data, offset, length);
        }
    }
    ...
    private Volume parseVolumeJson(String jsonStr) {
        ...
        try {
            ...
            vol.mVolumePercent = json.getInt("volume-percent");
            vol.mVolume = json.getInt("volume");
