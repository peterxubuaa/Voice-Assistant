...
import com.min.aiassistant.baidu.speech.listener.MessageStatusRecogListener;
...
import static com.min.aiassistant.baidu.speech.IStatus.STATUS_FINAL_RESULT;
import static com.min.aiassistant.baidu.speech.IStatus.STATUS_FINISHED;
import static com.min.aiassistant.baidu.speech.IStatus.STATUS_FINISHED_ERROR;
import static com.min.aiassistant.baidu.speech.IStatus.STATUS_LONG_SPEECH_FINISHED;
import static com.min.aiassistant.baidu.speech.IStatus.STATUS_NONE;
import static com.min.aiassistant.baidu.speech.IStatus.STATUS_OFFLINE_LOAD;
import static com.min.aiassistant.baidu.speech.IStatus.STATUS_OFFLINE_UNLOAD;
import static com.min.aiassistant.baidu.speech.IStatus.STATUS_PARTIAL_FINISHED;
import static com.min.aiassistant.baidu.speech.IStatus.STATUS_READY;
import static com.min.aiassistant.baidu.speech.IStatus.STATUS_RECOGNITION;
import static com.min.aiassistant.baidu.speech.IStatus.STATUS_SPEAKING;
import static com.min.aiassistant.baidu.speech.IStatus.STATUS_STOPPED;
...
/*  PID	语言	模型	是否有标点	在线语义	备注
    ...
    public interface OnSpeechListener {
        void onExit();
        void onFinalResult(String result, boolean query);
    ...
    static class BaiduSpeechHandler extends Handler {
        ...
        BaiduSpeechHandler(BaiduSpeechAI speechAI) {
            mTheBaiduSpeechAI = new WeakReference<>(speechAI);
        }
...
//                STATUS_FINISHED 获得最终识别结果
                ...
                case STATUS_FINAL_RESULT:
                    ...
                    if (null != mTheBaiduSpeechAI.get().mListener) {
                        mTheBaiduSpeechAI.get().mListener.onFinalResult(String.valueOf(msg.obj), mTheBaiduSpeechAI.get().mQuery);
                    }
                    break;
    ...
    public void initBaiduSpeech() {
        if ((mWorkState & SPEECH_INIT_EXIT_MASK) > 0) return;
        //baidu speech
    ...
    //开始录音，点击“开始”按钮后调用。
    public void startBaiduSpeechRecognize(boolean query) {
        if (null == mBaiduSpeech_Recognizer) return;

        if (!isBaiduSpeechRecognizeRunning()) {
            final int VAD_TIMEOUT = 800;
